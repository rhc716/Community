...
//비밀번호 유효성 검사 통과시 일치 확인/* 여기부터 만들것 */
//영문 숫자 특문 6~20자
$(document).ready(function(){
	$('#pw').blur(function(){
...
<div class="row">
	...
	<div class="col-xs-6 form_page">
		...
		<!--ajax 호출할때 넣을 값을 위해 히든으로 insert_type을 저장해줌-->
		<input type="hidden" id="insert_type" name="insert_type" value="${param.insert_type}">
<!--
처음 가입할때 user_leveup과 user_level은 자동으로 기입되므로, hidden으로 넘김
그리고 상단 바의 차별화를 유지하기 위함.
 -->
<%
	if(insert_type.equals("firm")){
%>
		<input type="hidden" name="user_level" value="기업회원">
		<input type="hidden" name="insert_type" value="firm">
<%
	}else if(insert_type.equals("child")){
%>
		<input type="hidden" name="user_level" value="일반회원">
		<input type="hidden" name="insert_type" value="child">
<%
	}else {
%>
		<input type="hidden" name="user_level" value="일반회원">
		<input type="hidden" name="insert_type" value="general">
<%
	}
%>
	<input type="hidden" name="user_levelup" value="1">		  
